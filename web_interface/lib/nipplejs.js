  ...
  for (var i = 0; i < types.length; i += 1) {
    ...
    if (self._handlers_[type] && self._handlers_[type].length) {
      self._handlers_[type].forEach(function(handler) {
        handler.call(self, {type: type, target: self}, data);
      });
    }
  }
};
...
Nipple.prototype.restPosition = function(cb) {
  var self = this;
  self.frontPosition = {x: 0, y: 0};
  var animTime = self.options.fadeTime + 'ms';

  var transitStyle = {};
  transitStyle.front =
      u.getTransitionStyle('transition', ['top', 'left'], animTime);
...
Nipple.prototype.computeDirection = function(obj) {
  ...
  if (obj.force > this.options.threshold) {
    ...
    this.direction = {x: directionX, y: directionY, angle: direction};
...
Collection.prototype.createNipple = function(position, identifier) {
  ...
  if (position.x && position.y) {
    ...
    toPutOn = position;
    position = {x: dumbBox.left + scroll.x, y: dumbBox.top + scroll.y};
  }
...
Collection.prototype.processOnMove = function(evt) {
  ...
  nipple.frontPosition = {x: xPosition, y: yPosition};
  ...
  var toSend = {
    ...
    distance: dist,
    angle: {radian: rAngle, degree: angle},
    instance: nipple,
    lockX: opts.lockX,
    lockY: opts.lockY
  };
  ...
  toSend.angle = {radian: u.radians(180 - angle), degree: 180 - angle};
    ...
    resizeTimer = setTimeout(function() {
      ...
      self.collections.forEach(function(collection) {
        collection.forEach(function(nipple) {
          ...
          nipple.position = {x: scroll.x + pos.left, y: scroll.y + pos.top};
        });
      });
    }, 100);
  });
